...
4. They try `research/1000-orgs/batches/` — see 12 batch JSON files, an assemble.py, a 174K CSV, a 20K SYNTHESIS.md, **and zero explanation of which file is what**. Someone has to read `assemble.py` to understand the data flow.
...
| 5 | 12 batch JSONs without an index | `research/1000-orgs/batches/` looks like chaos |
...
| 12 | No `governance/` folder | This document had nowhere to live |
...
- **Outbound deliverables** (`pitch/`, `outreach/`, `tech-spec/`) live separately from research. This is what we *send* to people vs what we *learned*.
...
6. **Add new content folders** (pitch/, outreach/, tech-spec/, financial/, strategy/, policy/, competitive/) with stubs + first-draft content for the critical ones
...
26. **`financial/cost-estimate.md`** — Cost to implement each new revenue stream
...
1. **Are you OK with this restructure** (numbered prefixes, archive of originals in `_originals/`, atomic commit)? It's reversible — nothing is deleted, just moved.
